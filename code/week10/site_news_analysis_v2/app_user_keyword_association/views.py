...
def filter_dataFrame_fullText(user_keywords, cond, cate, weeks):
    ...
    if cate == "全部":
        condition = period_condition  # no filtering on category
    else:
        condition = period_condition & (df.category == cate)

    # (3) proceed filtering: keyword matching
    if cond == 'and':
        condition = condition & df.content.apply(
            lambda text: all(qk in text for qk in user_keywords)
            if isinstance(text, str) else False
        )
    elif cond == 'or':
        condition = condition & df.content.apply(
            lambda text: any(qk in text for qk in user_keywords)
            if isinstance(text, str) else False
        )
